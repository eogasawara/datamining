...
Além de preparar treino/teste, esta seção define `eval_model()`, que padroniza avaliação entre algoritmos heterogêneos.
Isso é importante porque alguns modelos retornam rótulos e outros retornam probabilidades.

### Como funciona o `eval_model()`
Resumo da lógica:
1. tenta `evaluate()` diretamente;
2. se necessário, converte saída para um formato compatível com as classes-alvo;
3. reaplica avaliação com um classificador-proxy;
4. reporta métricas para treino e teste com o mesmo protocolo.
...
Nesta seção, os métodos cobrem abordagens complementares:
1. filtro por informação/correlação;
2. método wrapper (stepwise);
3. regularização (LASSO);
4. método de vizinhança (RELIEF).
...
Leitura sugerida:
1. desbalanceamento: veja efeito da reamostragem no ajuste;
2. semi-supervisão: compare antes/depois de pseudo-rótulos;
3. transferência: compare modelo pré-treinado vs ajuste em subconjunto.
